'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
ProcStat.py
Describe: Define a class ProcStat, this class can read files in filesystem /proc,
          then convert to python objects
Input: None
Output: Stats and information, include CPU, Memeory, Swap, Process and modules
Date: 2013/6/13
Version: python2.7.4
Author: Weiwei Jiang
E-mail: weiweijiangcn@gmail.com
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
class ProcStat():
	...
	def getCPUInfo(self):
		...
		for x in xrange(0, len(listCPUInfo) - 1):
			if len(cpuinfo) == 3:
				break
	...
	def getOSInfo(self):
		...
		for x in xrange(0, len(listOSInfo)):
			if len(osinfo) == 3:
				break;
	...
	def getMemInfo(self):
		...
		for x in xrange(0, len(listMemInfo)):
			if len(meminfo) == 4:
				break
	...
	def getProcInfos(self):
		...
		sleeping  = 0
		defunct   = 0
		for x in xrange(0, len(self.strProcs)):
			procinfo = self.strProcs[x].split()
			procinfos += [{"pid":procinfo[0], "name":procinfo[1][1:-1].ljust(20), "status":procinfo[2], 
							"ppid":procinfo[3], "priority":procinfo[17], "memory":procinfo[22]}]
			total += 1
			if procinfo[2] == "R":
				runnable += 1
				continue
	...
	def getModuleInfos(self):
		...
		for x in xrange(0, len(listModInfo)):
	...
	def getCPUStat(self, gapTime = 0.1):
		...
		for x in xrange(0, len(listCPUStat)):
